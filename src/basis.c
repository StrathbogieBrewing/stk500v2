...
void programcmd(unsigned char seqnum) {
    ...
    case CMD_SET_PARAMETER:
        ...
        if (msg_buf[1] == PARAM_SCK_DURATION) {
            stk_spi_set_sck_duration(msg_buf[2]);
    ...
    case CMD_ENTER_PROGMODE_ISP: // 0x10
        ...
        while (i < msg_buf[4]) { // synchLoops
            watchdog_reset();
            led_set(true);        // blink during init
            delay_ms(msg_buf[3]); // cmdexeDelay
            i++;
            stk_spi_mastertransmit(msg_buf[8]);         // cmd1
            delay_ms(msg_buf[5]);                   // byteDelay
            stk_spi_mastertransmit(msg_buf[9]);         // cmd2
            delay_ms(msg_buf[5]);                   // byteDelay
            led_set(false);                         // blink during init
            tmp = stk_spi_mastertransmit(msg_buf[10]);  // cmd3
            delay_ms(msg_buf[5]);                   // byteDelay
            tmp2 = stk_spi_mastertransmit(msg_buf[11]); // cmd4
            //
            ...
            if (msg_buf[1] == STATUS_CMD_OK) {
                ...
                // Guido thinks that stk_spi_sck_pulse is correct
    ...
    case CMD_CHIP_ERASE_ISP:
        stk_spi_mastertransmit(msg_buf[3]);
        stk_spi_mastertransmit(msg_buf[4]);
        stk_spi_mastertransmit(msg_buf[5]);
        stk_spi_mastertransmit(msg_buf[6]);
        if (msg_buf[2] == 0) {
            ...
            ci = 150; // timeout
            while ((stk_spi_mastertransmit_32(0xF0000000) & 1) && ci) {
                ci--;
            }
        }
        answerlen = 2;
        ...
        if ((msg_buf[3] & 1) == 0) {
            ...
            for (i = 0; i < nbytes; i++) {
                ...
                if (addressing_is_word && i & 1) {
                    // high byte
                    stk_spi_mastertransmit(msg_buf[5] | (1 << 3));
                } else {
                    // low byte
                    stk_spi_mastertransmit(msg_buf[5]);
                }
                stk_spi_mastertransmit_16(address & 0xFFFF);
                stk_spi_mastertransmit(msg_buf[i + 10]);
                ...
                if (msg_buf[3] & 0x04) {
                    ...
                    while (tmp == msg_buf[8] && ci) {
                        ...
                        // Read data:
                        if (addressing_is_word && i & 1) {
                            stk_spi_mastertransmit(msg_buf[7] | (1 << 3));
                        } else {
                            stk_spi_mastertransmit(msg_buf[7]);
                        }
                        stk_spi_mastertransmit_16(poll_address);
                        tmp = stk_spi_mastertransmit(0x00);
                        ci--;
                    }
            ...
            for (i = 0; i < nbytes; i++) {
                ...
                if (larger_than_64k && ((address & 0xFFFF) == 0 || new_address)) {
                    ...
                    stk_spi_mastertransmit(0x4d);
                    stk_spi_mastertransmit(0x00);
                    stk_spi_mastertransmit(extended_address);
                    stk_spi_mastertransmit(0x00);
                    new_address = 0;
                }
                // The Low/High byte selection bit is
            ...
            if (msg_buf[3] & 0x80) {
                stk_spi_mastertransmit(msg_buf[6]);
                stk_spi_mastertransmit_16(saddress);
                stk_spi_mastertransmit(0);
                //
                if (!addressing_is_word) {
                    // eeprom writing, eeprom needs more time
                    delay_ms(1);
    ...
    case CMD_READ_FLASH_ISP:
        ...
        for (i = 0; i < nbytes; i++) {
            ...
            if (addressing_is_word && i & 1) {
                stk_spi_mastertransmit(tmp | (1 << 3));
            } else {
                stk_spi_mastertransmit(tmp);
            }

            stk_spi_mastertransmit_16(address & 0xFFFF);
            msg_buf[i + 2] = stk_spi_mastertransmit(0);
    ...
    case CMD_PROGRAM_FUSE_ISP:
        stk_spi_mastertransmit(msg_buf[1]);
        stk_spi_mastertransmit(msg_buf[2]);
        stk_spi_mastertransmit(msg_buf[3]);
        stk_spi_mastertransmit(msg_buf[4]);
        answerlen = 3;
    ...
    case CMD_READ_FUSE_ISP:
        for (ci = 0; ci < 4; ci++) {
            tmp = stk_spi_mastertransmit(msg_buf[ci + 2]);
            if (msg_buf[1] == (ci + 1)) {
                msg_buf[2] = tmp;
            }
            delay_ms(5);
    ...
    case CMD_SPI_MULTI:
        ...
        for (cj = 0; cj < msg_buf[1]; cj++) {
            ...
            if (cj >= tmp2 && ci < tmp) {
                ...
                msg_buf[ci + 2] = stk_spi_mastertransmit(msg_buf[cj + 4]);
                ci++;
            } else {
                stk_spi_mastertransmit(msg_buf[cj + 4]);
            }
        }
        // padd with zero:
        while (ci < tmp) {
